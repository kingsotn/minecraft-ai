...
class CurriculumAgent:
    ...
    def render_observation(self, *, events, chest_observation):
        ...
        other_blocks = ", ".join(
            list(set(block_records).difference(set(voxels).union(set(inventory.keys()))))
        )
    ...
    def render_human_message(self, *, events, chest_observation):
        ...
        if self.progress >= self.warm_up["context"]:
            questions, answers = self.run_qa(events=events, chest_observation=chest_observation)
            i = 1
    ...
    def propose_next_task(self, *, events, chest_observation, max_retries=5):
        ...
        if self.progress == 0 and self.mode == "auto":
            task = "Kill things"
            context = (
                "You can kill any mob or animals such as pigs, sheep, chicken or harm villagers"
            )
            return task, context
